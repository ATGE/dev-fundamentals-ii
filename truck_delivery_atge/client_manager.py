...
class ClientManager(ContentManager):
    ...
    def save_document(self, ci, client: Client):
        save_result = self.db_connector.save(f'{CLIENT_ID}-{ci}', client.to_dict())
        return save_result

    def get_document(self, ci):
        get_result = self.db_connector.get_by_id(f'{CLIENT_ID}-{ci}')
        return get_result

    def get_all(self):
        get_result = self.db_connector.get_all(f'{CLIENT_ID}')
        return get_result

    def delete(self, ci):
        return self.db_connector.delete_by_id(f'{CLIENT_ID}-{ci}')
